// ColorDlgDlg.cpp : implementation file
//
...
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CColorDlgDlg)
	DDX_Text(pDX, IDC_EDIT1, m_strEdit1);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CColorDlgDlg, CDialog)
	//{{AFX_MSG_MAP(CColorDlgDlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
BOOL CColorDlgDlg::OnInitDialog()
{
	...
	m_strEdit1.Format("Red: %d, Green: %d, Blue: %d",
		               GetRValue(m_Color),
					   GetGValue(m_Color),
					   GetBValue(m_Color));
	UpdateData(FALSE);
